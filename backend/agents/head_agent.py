...
class HeadAgent:
    ...
    def simulate_whatif(self, scenario: WhatIfScenario) -> WhatIfScenario:
        """
        Apply a hypothetical scenario to the last known anomaly set and re-score.

        Uses the cached ``last_report`` from each specialist (not a fresh run) to
        keep latency low. Severity multipliers are applied per domain before the
        full analyze() pipeline is re-executed. A two-sentence comparison briefing
        is generated by Gemini comparing the current vs. simulated risk states.

        Args:
            scenario: ``WhatIfScenario`` with ``scenario_type`` and ``parameters``.
                      Populated in place with simulation results before being returned.
        """
